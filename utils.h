...
void output_ppm (scalar f, double min, double max, 
		 int n, FILE * fp, bool linear)
{
  fprintf (fp, "P6\n%u %u 255\n", n, n);
  float cmap[NCMAP][3];
  colormap_jet (cmap);
  ...
  for (int j = n - 1; j >= 0; j--) {
    double y = delta*j + Y0 + delta/2.;
    for (int i = 0; i < n; i++) {
      double x = delta*i + X0 + delta/2., v;
      if (linear)
	v = interpolate (f, x, y);
      else {
	Point point = locate (x, y);
	v = point.level >= 0 ? val(f,0,0) : nodata;
      }
      unsigned char c[3];
      if (v == nodata)
	c[0] = c[1] = c[2] = 0;
      else
	colormap_color (cmap, v, min, max, c);
