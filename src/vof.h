...
event vof_advection (i = 1; i++)
{
  for (scalar c in interfaces) {
    ...
    // fixme: generalise foreach_dimension() to automatically derive this one
    reconstruction (c, n, alpha);
    trash ({flux});
    foreach_face(y) {
      double un = u.y[]*dt/Delta, s = sign(un);
      int i = -(s + 1.)/2.;
      double cf = (c[0,i] <= 0. || c[0,i] >= 1.) ? c[0,i] :
	rectangle_fraction (c[0,i], 
			    - s*n.y[0,i], n.x[0,i],
			    alpha[0,i] - (s + 1.)/2.*n.y[0,i],
			    0, 0, s*un, 1);
      flux[] = u.y[]*cf;
    }
    //    boundary_normal ({flux});
    foreach()
      c[] += dt*(flux[] - flux[0,1] + cc[]*(u.y[0,1] - u.y[]))/Delta;
    boundary ({c});
