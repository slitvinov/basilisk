...
The Euler system of conservation laws for a compressible gas with
polytropic exponent $\gamma$ can be written
...
gas momentum and $p$ the pressure given by

$$
p = (\gamma - 1)(E - \rho\mathbf{u}^2/2)
$$

This system can be solved using the generic solver for systems of
conservation laws. */
...
$E$. The only conserved vector is the momentum $w$.  The constant
...
($\rho$, $E$, $w_x$ and $w_y$) and then compute the corresponding
fluxes (`f[0]`, `f[1]`, `f[2]` and `f[3]`). The minimum and maximum
eigenvalues for the Euler system are the characteristic speeds $u \pm
\sqrt(\gamma p / \rho)$. */
